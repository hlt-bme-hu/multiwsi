...
def neela_filter(inembed_fn, global_fn, sense_fn, ccent_fn):
    with gzip.open(inembed_fn) as inembed_f, \
        ...
        if len(header) == 4:
            vocab_size, dim, max_sense, minus = (int(str_) for str_ in header)
            vec_per_sense = max_sense - minus
        else:
        ...
        while True:
            ...
            if line:
                count +=1
                if not count % 10000:
                    sys.stdout.write('\rProgress: {:.1%}'.format(float(count)/vocab_size))
                    sys.stdout.flush()
                if line.startswith(' '):
